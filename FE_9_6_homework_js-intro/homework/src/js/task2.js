const length_1 = parseFloat(prompt('Enter lenght of first side', '0'));
const length_2 = parseFloat(prompt('Enter discount', '0'));
const angle = parseFloat(prompt('Enter angle between sides', '0'));

if ( typeof length_1 !== 'number' || typeof length_2 !== 'number' || typeof angle !== 'number') {
	console.log('Invalid data')
} else {
	if ( length_1 > 0 && length_2 > 0 && angle > 0) {
		const length_3 = getThirdSide();
		const perimeter = getPerimeter(length_3);
		const square = getSquare(perimeter, length_3);
...
function getThirdSide(){
	...
	const rad = Math.PI / sumOfSides * angle;
